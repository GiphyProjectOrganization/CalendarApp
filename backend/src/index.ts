...
app.post(
  ...
  async (
    ...
    if (!/\d/.test(password) || !/[!@#$%^&*(),.?":{}|<>]/.test(password)) {
      res
        .status(400)
        .json({ message: 'Password must have one number and one symbol!' });
      return;
    }
    if (
      firstName.length < 1 ||
      ...
      res
        ...
        .json({
          message:
            'First name must be between 1 and 30 characters and include only letters!',
        });
      return;
    }
    if (
      ...
      res
        ...
        .json({
          message:
            'Last name must be between 1 and 30 characters and include only letters!',
        });
      return;
    }
    if (phoneNumber.length !== 10 || !/^\d+$/.test(phoneNumber)) {
      res
        .status(400)
        .json({
          message: 'Phone number must be 10 digits and include only numbers!',
        });
      return;
    }
    ...
    try {
      ...
      try {
        await session.withTransaction(async () => {
          ...
          const contactListsCollection =
            db.collection<ContactList>('contactLists');
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    if (
      ...
      if (!startDate || !startTime || !endDate || !endTime) {
        res
          .status(400)
          .json({ message: 'Start and end date/time are required' });
        return;
      }
    ...
    try {
      ...
      res
        .status(500)
        .json({ message: 'Failed to update event. Please try again.' });
    }
  }
);
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      if (typeof photoBase64 === 'string' && photoBase64.trim())
        updateFields.photoBase64 = photoBase64.trim();
...
app.get(
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      const contactLists = await contactListsCollection
        .find({
          owner: userObjectId,
        })
        .toArray();
...
app.get('/api/admin/stats', adminMiddleware, async (req, res) => {
  try {
    ...
    const draftEvents = await eventsCollection.countDocuments({
      isDraft: true,
    });
    const publicEvents = await eventsCollection.countDocuments({
      isPublic: true,
    });
...
app.put('/api/admin/events/:eventId', adminMiddleware, async (req, res) => {
  const { eventId } = req.params;
  const updates = req.body;
  ...
  try {
    ...
    if (
      ...
      const startDateTime = new Date(
        `${updates.startDate}T${updates.startTime}`
      );
